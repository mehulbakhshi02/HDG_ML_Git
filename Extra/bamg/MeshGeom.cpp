  ...
  Real8 GeometricalEdge::R1tg(Real8 theta, R2 &t) const    // 1/R of radius of cuvature
  {
    ...
    if (TgA( ))
      ...
      //   ddctb = 6 * theta - 2;
       ...
       R2 wtg2 = tg[1]/wt;
       R2 f;
       f.x = A.y*wtg1.x-wtg1.y*A.x;
       f.y = B.y*wtg2.x-wtg2.y*B.x;
       Real8 detinv = -wtg1.x*wtg2.y+wtg1.y*wtg2.x;
       if(fabs(detinv)>1e-10)
       {
           R2 mid;
           mid.x = (f.y*wtg1.x-wtg2.x*f.x)/detinv;
           mid.y = (f.y*wtg1.y-wtg2.y*f.x)/detinv;
           Real8 d1n = Norme2(A-mid)*Norme2(A-mid);
           Real8 d2n = Norme2(B-mid)*Norme2(B-mid);
           Real8 weight = wt/sqrt(0.5*(d1n+d2n));
           Real8 b1 = (1.-theta)*(1.-theta);
           Real8 b2 = weight * theta * (1.-theta);
           Real8 b3 = theta * theta;
           Real8 w = b1+b2+b3;
           b1 = b1/w;
           b2 = b2/w;
           b3 = b3/w;
           ...
           Real8 b1p = 2.*(theta-1.);
           Real8 b2p = weight*(1.-2.*theta);
           Real8 b3p = 2.*theta;
           Real8 wp = b1p+b2p+b3p;
           Real8 fac1 = wp/w;
           b1p = b1p/w;
           b2p = b2p/w;
           b3p = b3p/w;
           ...
           Real8 b2pp = -2.*weight;
           Real8 b3pp = 2.;
           Real8 wpp = b1pp+b2pp+b3pp;
           Real8 fac2 = (wpp*w-2.*wp*wp)/(w*w);

           R2 d = A * (b1p-b1*fac1) + mid * (b2p-b2*fac1) + B * (b3p-b3*fac1);
           R2 dd = A * (b1pp/w - 2.*b1p*fac1-b1*fac2) + mid * (b2pp/w-2.*b2p*fac1-b2*fac2) + B * (b3pp/w-2.*b3p*fac1-b3*fac2);
           
           Real8 d2 = (d, d);
           Real8 sd2 = sqrt(d2);
           t = d;
           // R2 point = (A * b1 + mid * b2 + B * b3)/tot;
           // return point;
    ...
    if (TgA( ))
      if (TgB( ))    // interpolation d'hermite
      {
       ...
       if(fabs(detinv)>1e-10)
       {
         ...
         mid.x = (f.y*wtg1.x-wtg2.x*f.x)/detinv;
         mid.y = (f.y*wtg1.y-wtg2.y*f.x)/detinv;
         Real8 d1 = Norme2(A-mid)*Norme2(A-mid);
         Real8 d2 = Norme2(B-mid)*Norme2(B-mid);
         Real8 weight = w/sqrt(0.5*(d1+d2));
         Real8 b1 = (1.-theta)*(1.-theta);
         Real8 b2 = weight * theta * (1.-theta);
         Real8 b3 = theta * theta;
         Real8 tot = b1+b2+b3;
