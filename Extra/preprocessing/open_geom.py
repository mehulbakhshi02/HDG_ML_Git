
import numpy as np 
import csv
import matplotlib.pyplot as plt
import random
import time
from netgen.geom2d import SplineGeometry
from netgen import meshing
from netgen.meshing import Mesh, FaceDescriptor, Element3D,Element2D, Element1D
def ReadPoints(filename):
	x, y=np.loadtxt(filename, delimiter=',', unpack=True)
	return x, y

def WritePointsBumpTMR(filename):
	nps_bump = 100
	x_start = 0.3
	x_end = 1.2
	x_bump = np.linspace(x_start,x_end, nps_bump)
	y_bump = 0.05*(np.sin(np.pi*x_bump/0.9-(np.pi/3.)))**4
	x = x_bump
	y = y_bump
	write_string = filename

	with open(write_string, 'w') as f:
	    writer = csv.writer(f)
	    for i in range(0, len(x)):
	    	writer.writerow((x[i], y[i]))
def WritePointsBumpHO(filename):
	nps_bump = 100
	x_start = -1.1
	x_end = 1.1
	x_bump = np.linspace(x_start,x_end, nps_bump)
	y_bump = 0.0625*np.exp(-25.*x_bump*x_bump)
	x = x_bump
	y = y_bump
	write_string = filename

	with open(write_string, 'w') as f:
	    writer = csv.writer(f)
	    for i in range(0, len(x)):
	    	writer.writerow((x[i], y[i]))

def ComputeCoefficients(y):

	n_coeff = 3
	m = len(y)
	f = np.zeros(m)
	A = np.zeros(m*m)

	A[0] = 2.
	A[1] = 1.
	A[m*(m-1)+(m-2)] = 1
	A[m*(m-1)+(m-1)] = 2
	f[0] = 3.*(y[1]-y[0])
	f[len(y)-1] = 3.*(y[-1]-y[-2])
	for i in range(1, m-1):
		f[i] = 3.*(y[i+1]-y[i-1])
		A[i*m+i] = 4
		A[i*m+i-1] = 1
		A[i*m+i+1] = 1
	# return U
	A = np.reshape(A, (m, m))

	# print(f)
	Ainv = np.linalg.inv(A) 
	D = Ainv.dot(f)

	U = np.zeros(4*(m-1))
	for i in range(0, m-1):
		a = y[i]
		b = D[i]
		c = 3.*(y[i+1]-y[i])-2.*D[i]-D[i+1]
		d = 2.*(y[i]-y[i+1])+D[i]+D[i+1]

		ap = i*4+0
		bp = i*4+1
		cp = i*4+2
		dp = i*4+3

		U[ap] = a
		U[bp] = b
		U[cp] = c
		U[dp] = d
	return U

def PlotCurve(Ux,Uy, x, y):

	plt.plot(x, y, 'r*')
	# plt.plot(x, 'bo')
	N = 50
	tg = np.linspace(0, 1, N)
	x_c = []
	y_c = []
	# y_c.append(0)
	# x_c.append(0)
	for i in range(0, len(x)-1):
		ax = Ux[i*4+0]
		bx = Ux[i*4+1]
		cx = Ux[i*4+2]
		dx = Ux[i*4+3]
		ay = Uy[i*4+0]
		by = Uy[i*4+1]
		cy = Uy[i*4+2]
		dy = Uy[i*4+3]
		for j in range(0, N):
			t = tg[j]
			x_loc = ax+bx*t+cx*t*t+dx*t*t*t
			y_loc = ay+by*t+cy*t*t+dy*t*t*t
			x_c.append(x_loc)
			y_c.append(y_loc)

	plt.plot(x_c, y_c, 'r-')
def CalcMid(Ux, Uy, x, y):
	m = len(x)
	x_m = []
	y_m = []
	x_full = []
	y_full = []
	for i in range(0, m-1):
		ax = Ux[i*4+0]
		bx = Ux[i*4+1]
		cx = Ux[i*4+2]
		dx = Ux[i*4+3]
		ay = Uy[i*4+0]
		by = Uy[i*4+1]
		cy = Uy[i*4+2]
		dy = Uy[i*4+3]
		dxdt1 = bx
		dydt1 = by
		# dxdt1 = max(abs(dxdt1), 1e-15)
		dydx1 = dydt1/dxdt1

		dxdt2 = bx+2.*cx+3.*dx
		dydt2 = by+2.*cy+3.*dy
		# dxdt2 = max(dxdt2, 1e-15)
		dydx2 = dydt2/dxdt2

		m1 = dydx1
		m2 = dydx2
		x1 = x[i]
		y1 = y[i]
		x2 = x[i+1]
		y2 = y[i+1]
		c1 = y1-m1*x1
		c2 = y2-m2*x2
		if(abs(y1)<1e-8):
			x_mid=(x1)
			y_mid=(m2*x1+c2)
		elif(abs(y2)<1e-8):
			x_mid=(x2)
			y_mid=(m1*x2+c1)
		else:
			x_mid=((c1-c2)/(m2-m1))
			y_mid=((m2*c1-m1*c2)/(m2-m1))
		# Hack for cases with inflection point like rae
		if((x_mid>x1 and x_mid>x2) or (x_mid<x1 and x_mid<x2)):
			x_mid = 0.5*(x1+x2)
			y_mid = 0.5*(y1+y2)

		x_m.append(x_mid)
		y_m.append(y_mid)
		x_full.append(x1)
		x_full.append(x_mid)
		y_full.append(y1)
		y_full.append(y_mid)

		# if((y_mid>y1 and y_mid>y2) or (y_mid<y1 and y_mid<y2)):
		# 	print(y1, y2, y_mid)
	x_full.append(x[-1])
	y_full.append(y[-1])
	plt.plot(x_full, y_full, 'o')
	plt.plot(x_m, y_m, '*')
	return x_full, y_full
def WriteGeomFileBumpTMR(x_full, y_full, domain, filename):
	nps = 1
	# x_out = [-domain/2, 4.,  domain, domain, -domain/2]
	# y_out = [-domain/2, -domain/2,  -domain/2, domain/2, domain/2]
	lower_sym_1 = np.linspace(-25., 0, nps, endpoint=False)
	lower_wall_x = np.linspace(0,0.3, nps, endpoint=False)
	lower_wall_x_2 = np.linspace(1.2,1.5, nps)
	lower_sym_2 = np.linspace(1.5, 26.5, nps, endpoint=False)
	upper_wall = np.linspace(26.5, -25, nps, endpoint=False)
	outlet = np.linspace(0.0, 5, nps, endpoint=False)
	inlet = np.linspace(5, 0, nps, endpoint=False)


	nps_bump = len(x_full)
	tot_points = 6*nps+nps_bump
	count = 1
	order = 2
	print(lower_wall_x, x_full)
	# Try and write the geometry file for netgen
	f = open(filename+".in2d", "w")
	# Key word for the 2d geometry file
	f.write('splinecurves2dv2\n')
	# Grading factor
	f.write('1\n')
	# Number of points in the geometry
	f.write('points\n')
	# Write the lower wall sym
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(lower_sym_1[i]) + '\t\t' + str(0.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the lower wall adb
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(lower_wall_x[i]) + '\t\t' + str(0.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the lower wall bump
	for i in range(0, nps_bump):
		line = str(count) + '\t\t' + str(x_full[i]) + '\t\t' + str(y_full[i]) + '\n'
		count = count + 1
		f.write(line)
	# Write the lower wall adb
	for i in range(1, nps):
		line = str(count) + '\t\t' + str(lower_wall_x_2[i]) + '\t\t' + str(0.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the lower wall sym
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(lower_sym_2[i]) + '\t\t' + str(0.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the outlet
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(26.5) + '\t\t' + str(outlet[i]) + '\n'
		count = count + 1
		f.write(line)
	# Write the outlet
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(upper_wall[i]) + '\t\t' + str(5.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the inlet
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(-25.) + '\t\t' + str(inlet[i]) + '\n'
		count = count + 1
		f.write(line)
	# Number of segments in the geometry
	f.write('\nsegments\n')
	count = 0
	# We want to write the sym
	for i in range(0, int((nps))):
		bc_val = 3
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the adb
	for i in range(0, int((nps))):
		bc_val = 2
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	for j in range(0, int(nps_bump/order)):
		# # We want to write the lower wall with 1st order geometry
		for i in range(0, int((1))):
			bc_val = 2
			line = str('1') + '\t\t' + '0' + '\t\t' + str(order+1) + '\t\t'
			for o in range(0, order+1):
				line = line + str((count+o)%tot_points+1) + '\t\t' 
			line = line  + '-bc='+str(bc_val)+'\n'
			count = count + order
			f.write(line)
	for i in range(0, int((nps))):
		bc_val = 2
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the sym
	for i in range(0, int((nps))):
		bc_val = 3
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the outlet
	for i in range(0, int((nps))):
		bc_val = 5
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the sym
	for i in range(0, int((nps))):
		bc_val = 3
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the inlet
	for i in range(0, int((nps))):
		bc_val = 4
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	f.write('\n\nmaterials\n1\t\tdomain1')
def WriteGeomFileBumpHO(x_full, y_full, domain, filename):
	nps = 1
	# x_out = [-domain/2, 4.,  domain, domain, -domain/2]
	# y_out = [-domain/2, -domain/2,  -domain/2, domain/2, domain/2]
	lower_sym_1 = np.linspace(-1.5, -1.17, nps, endpoint=False)
	lower_sym_2 = np.linspace(1.17, 1.5, nps, endpoint=False)
	upper_wall = np.linspace(1.5, -1.5, nps, endpoint=False)
	outlet = np.linspace(0.0, 0.8, nps, endpoint=False)
	inlet = np.linspace(0.8, 0, nps, endpoint=False)


	nps_bump = len(x_full)
	tot_points = 4*nps+nps_bump
	count = 1
	order = 2

	# Try and write the geometry file for netgen
	f = open(filename+".in2d", "w")
	# Key word for the 2d geometry file
	f.write('splinecurves2dv2\n')
	# Grading factor
	f.write('1\n')
	# Number of points in the geometry
	f.write('points\n')
	# Write the lower wall sym
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(round(lower_sym_1[i],20)) + '\t\t' + str(0.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the lower wall bump
	for i in range(0, nps_bump):
		line = str(count) + '\t\t' + str(round(x_full[i],20)) + '\t\t' + str(round(y_full[i],20)) + '\n'
		count = count + 1
		f.write(line)
	# Write the lower wall sym
	for i in range(1, nps):
		line = str(count) + '\t\t' + str(round(lower_sym_2[i],20)) + '\t\t' + str(0.0) + '\n'
		count = count + 1
		f.write(line)
	# Write the outlet
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(1.5) + '\t\t' + str(round(outlet[i],20)) + '\n'
		count = count + 1
		f.write(line)
	# Write the outlet
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(round(upper_wall[i],20)) + '\t\t' + str(0.8) + '\n'
		count = count + 1
		f.write(line)
	# Write the inlet
	for i in range(0, nps):
		line = str(count) + '\t\t' + str(-1.5) + '\t\t' + str(round(inlet[i],20)) + '\n'
		count = count + 1
		f.write(line)
	# Number of segments in the geometry
	f.write('\nsegments\n')
	count = 0
	# We want to write the sym
	for i in range(0, int((nps))):
		bc_val = 2
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	for j in range(0, int(nps_bump/order)):
		# # We want to write the lower wall with 1st order geometry
		for i in range(0, int((1))):
			bc_val = 2
			line = str('1') + '\t\t' + '0' + '\t\t' + str(order+1) + '\t\t'
			for o in range(0, order+1):
				line = line + str((count+o)%tot_points+1) + '\t\t' 
			line = line  + '-bc='+str(bc_val)+'\n'
			count = count + order
			f.write(line)
	for i in range(0, int((nps))):
		bc_val = 2
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the outlet
	for i in range(0, int((nps))):
		bc_val = 5
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the sym
	for i in range(0, int((nps))):
		bc_val = 2
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	# We want to write the inlet
	for i in range(0, int((nps))):
		bc_val = 4
		line = str('1') + '\t\t' + '0' + '\t\t' + str(2) + '\t\t' + str((count)%tot_points+1) + '\t\t' + str((count+1)%tot_points+1) + '\t\t'+'-bc='+str(bc_val)+'\n'
		count = count + 1
		f.write(line)
	f.write('\n\nmaterials\n1\t\tdomain1')
solve = time.time()
name = 'bump_ho.csv'
# name = 'naca_def.csv'
WritePointsBumpHO(name)
# WritePointsBumpTMR(name)
# Read in the data, synthetic or otherwise
x, y= ReadPoints(name)
# plt.plot(x, y, 'b')
xc=x
yc=y
Ux = ComputeCoefficients(xc)
Uy = ComputeCoefficients(yc)
PlotCurve(Ux, Uy, xc, yc)
x_full, y_full = CalcMid(Ux, Uy, xc, yc)#x_full, y_full = 
# x_mid1 = 0.5*(x[0]+x_full[0])
# y_mid1 = 0.5*(y[0]+y_full[0])
# x_mid_end = 0.5*(x[0]+x_full[-1])
# y_mid_end = 0.5*(y[0]+y_full[-1])
x_fin = []
# x_fin.append(x[0])
# x_fin.append(x_mid1)
y_fin = []
# y_fin.append(y[0])
# y_fin.append(y_mid1)
for i in range(0, len(x_full)):
	x_fin.append(x_full[i])
	y_fin.append(y_full[i])
# x_fin.append(x_mid_end)
# y_fin.append(y_mid_end)

geo_name = name[:-4]
WriteGeomFileBumpHO(x_fin, y_fin, 100, geo_name)
# WriteGeomFileBumpTMR(x_fin, y_fin, 100, geo_name)
geo = SplineGeometry(geo_name+".in2d")
mesh = geo.GenerateMesh(maxh=1000000000000000000000000.0)
mesh.Save(geo_name+".vol")

# plt.show()
# end = time.time()
# print(end-solve)


