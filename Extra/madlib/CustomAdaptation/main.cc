...
void DefineMet(pVertex ver, MAdMetric& met) {
  ...
  V_coord(ver, xyz);
  lgt[0] = 0.1;
  lgt[1] = 0.001+0.198*fabs(xyz[1]-0.5);
  lgt[2] = 0.;
  ...
  met = MAdMetric(1./(lgt[0]*lgt[0]), 1./(lgt[1]*lgt[1]), 1., dir[0], dir[1], dir[2]);
}
...
int main ( int argc, char* argv[] ) {
  ...
  double vec[6] = {aa, bb, cc, 0.0, 0.0, 0.0};
  MAdMetric met(vec);
	// DefineMet(ver, met);
  ...
  // --- write outputs
  M_writeMesh(msh, "adapted.mesh", 2, 2);
