...
using std::pow;
template <int D, typename SCAL>
inline AutoDiff<D,SCAL> pow (const AutoDiff<D,SCAL> & x, SCAL val2)
{
  AutoDiff<D,SCAL> res(std::pow(x.Value(), val2));
  for (int k = 0; k < D; k++)
    res.DValue(k) = val2 * x.DValue(k) * std::pow(x.Value(), val2 - 1.);
  return res;
}
