...
hidden_layer_1 = Dense(number_neurons,activation=activation_func)(field_input)
for i in range(no_hidden_layers):
    hidden_layer_1 = Dense(number_neurons,activation=activation_func)(hidden_layer_1)
outputs = Dense(num_outputs,name='outputs')(hidden_layer_1)

model = Model(inputs=[field_input],outputs=[outputs])   
# tf.keras.utils.plot_model(model, 'simple_fcnn.png', show_shapes=True)

# Original
# my_adam = optimizers.Adam(lr=0.0001, decay=0.0)

# KUNAL
my_adam = optimizers.Adam(learning_rate=0.0001)
