...
void Netgen2Madlib2D(string netgen_mesh, string mmg_mesh) 
{
  ifstream netgen_in;
 ...
 for(int i=0;i<np;i++)
 {
   mmg<<i+1<<" "<<coord[i][0]<<" "<<coord[i][1]<<" "<<0.0<<endl;
 }
  mmg<<"$EndNodes"<<endl;
  mmg<<"$Elements"<<endl;
  mmg<<medges+ntng<<endl;
  j=0;
  while (j < medges){
    mmg <<j+1 <<" "<< 1 << " " << 3 << " "<< 0<< " " << surfid[j]<< " " << 0<< " " << epoints[j][0] << " " << epoints[j][1] << endl;
    j++;
  }
 for(int i=0;i<ntng;i++)
 {
   mmg<<medges+1+i<<" "<<2<<" "<<3<<" "<<0<<" "<<1<<" "<<0<<" "<<triang[i][0]<<" "<<triang[i][1]<<" "<<triang[i][2]<<endl;
 }
