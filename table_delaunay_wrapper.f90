subroutine table_delaunay_wrapper (node_num, table, triangle_num, triangle_node)

  implicit none

  integer, intent(in) :: node_num
  double precision, dimension(1:2*node_num), intent(inout) :: table
  integer, intent(in) :: triangle_num
  integer, dimension(1:9*node_num), intent(in) :: triangle_node

  call table_Delaunay(node_num, table, triangle_num, triangle_node)

end subroutine table_delaunay_wrapper

subroutine table_Delaunay (node_num, table, triangle_num, triangle_node)
...
!
  ...
  double precision :: reference_real

  integer, parameter :: real_kind = kind(reference_real)

  integer, parameter ::  m1=2
  integer m2
  integer node_num
  real ( kind = real_kind ), dimension ( m1 , node_num ) :: table
  integer, dimension ( 3, 3*node_num ) :: triangle_neighbor
  integer, dimension ( 3, 3*node_num ) :: triangle_node
...
!  call itable_data_print_some ( m2, triangle_num, triangle_node, 1, 1, 3, 5, &
...
!
!  eps_filename = input_filename
!  call file_name_ext_swap ( eps_filename, 'delaunay.eps' )

!  call triangulation_plot_eps ( eps_filename, node_num, table, &
...
subroutine dtable_data_print_some ( m, n, table, ilo, jlo, ihi, jhi, title )
...
!  Discussion:
!
!    The data is printed in transposed form.
!
!  Modified:
!
!    06 December 2003
...
!    write ( *, '(a)' ) ' '
    ...
    do j = j2lo, j2hi
      ...
      do i2 = 1, inc
        ...
        if ( d_is_int ( table(i,j) ) ) then
          write ( ctemp(i2), '(f8.0,6x)' ) table(i,j)
        else
          write ( ctemp(i2), '(g14.6)' ) table(i,j)
        end if
...
!
  ...
  do while ( i < n )
    ...
    call s_to_dvec ( line, m, x, ierror )
...
!
  call d2vec_sort_heap_index_a ( point_num, point_xy, indx )

  call d2vec_permute ( point_num, point_xy, indx )
!
...
!
  ...
  call word_count ( line, column_num )
...
subroutine itable_data_print_some ( m, n, table, ilo, jlo, ihi, jhi, title )
...
!  Parameters:
!
!    Input, character ( len = * ) INPUT_FILENAME, the output filename.
!
!    Input, character ( len = * ) OUTPUT_FILENAME, the output filename.
!
!    Input, integer M1, the spatial dimension of the nodes (must be 2).
!
!    Input, integer N1, the number of nodes.
!
!    Input, integer M2, the "spatial" dimension of the triangles (must be 3).
!
...
!
  ...
  write ( output_unit, '(a,i6)' ) '#  The spatial dimension was ', m1
  write ( output_unit, '(a,i6)' ) '#  The number of nodes was ', n1
  write ( output_unit, '(a)' )    '#'
...
subroutine s_to_d ( s, r, ierror, lchar )
...
subroutine s_to_dvec ( s, n, rvec, ierror )

!*******************************************************************************
!
!! S_TO_DVEC reads a double precision vector from a string.
!
!  Modified:
!
!    19 February 2001
...
!
  ...
  do while ( i < n )
    ...
    call s_to_d ( s(ilo:), rvec(i), ierror, lchar )
